<div align="center">

# 你好，我是 Rogue 👋

### 前端开发者 · AI 实践者 · 独立开发者

把想法做成真正能运行、能使用、能持续迭代的产品。

[![GitHub followers](https://img.shields.io/github/followers/RipWheeler-max?style=flat-square&logo=github&label=Followers)](https://github.com/RipWheeler-max?tab=followers)
[![X / Twitter](https://img.shields.io/badge/X-rogue30481577-000000?style=flat-square&logo=x&logoColor=white)](https://x.com/rogue30481577)

</div>

## 关于我

我是 99 年的程序员，有 3 年前端开发经验，现在主要探索 **Vibe Coding、AI 辅助开发和个人产品实践**。

- 💻 熟悉 Web 项目从页面实现、工程化开发到部署交付的完整流程
- 🤖 把 AI 接入需求拆解、方案设计、编码、调试、测试和发布，而不只是生成几段代码
- 🧩 喜欢把复杂问题拆成清晰的小步骤，快速做出可验证的 Demo，再持续打磨
- 🎬 正在实践 AI 视频生产、内容创作和个人知识管理等自动化工作流
- ✍️ 在 [X / Twitter](https://x.com/rogue30481577) 记录 AI 编程、独立开发和产品实践

## 代表作品

| 项目 | 简介 | 技术方向 |
| --- | --- | --- |
| [明史时间轴](https://github.com/RipWheeler-max/ming-history-timeline) · [在线体验](https://ming-history-timeline.roguesky360.chatgpt.site) | 沿着年份、皇帝、事件与人物关系，看懂整个明朝 | Next.js · TypeScript · 数据可视化 |
| [Claude Code Mastery Quest](https://github.com/RipWheeler-max/claude-code-mastery-quest) | 通过游戏化任务学习 Claude Code 的互动应用 | Next.js · TypeScript · AI 编程教育 |
| [Linux Quest](https://github.com/RipWheeler-max/linux-quest) | 用闯关方式练习 Linux 命令行 | TypeScript · 交互式学习 |
| [Gesture Universe Explorer](https://github.com/RipWheeler-max/gesture-universe-explorer) | 用手势控制的 3D 宇宙探索体验 | JavaScript · 3D · 手势交互 |
| [Personal Website](https://github.com/RipWheeler-max/personalwebsite) | 赛博朋克风格个人博客系统 | JavaScript · MongoDB · Vercel |
| [PiPix Website](https://github.com/RipWheeler-max/pipix-website) | 基于 FastAPI 与 yt-dlp 的视频下载工具 | Python · FastAPI · yt-dlp |

## 技术与工具

![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=000)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=fff)
![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=000)
![Vue](https://img.shields.io/badge/Vue-4FC08D?style=flat-square&logo=vuedotjs&logoColor=fff)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=fff)
![Node.js](https://img.shields.io/badge/Node.js-5FA04E?style=flat-square&logo=nodedotjs&logoColor=fff)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=fff)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=fff)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=flat-square&logo=tailwindcss&logoColor=fff)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=fff)

## 我正在探索

```text
一个想法
   ↓
需求拆解 → AI 协作开发 → 本地验证 → 部署上线 → 根据反馈继续迭代
```

- AI 编程 / Vibe Coding：让人负责方向、判断和验收，让 AI 加速执行
- 独立开发：快速验证小产品，并把原型打磨成真正可用的工具
- 自动化工作流：连接本地工具、内容生产、知识管理和发布流程
- 交互式学习：用游戏化、可视化方式降低复杂知识的学习门槛

## 联系我

如果你也在关注 **AI 编程、前端开发、独立产品或自动化工作流**，欢迎在 [X / Twitter](https://x.com/rogue30481577) 找我交流。

<div align="center">

<sub>持续构建，持续分享。Build in public.</sub>

</div>
